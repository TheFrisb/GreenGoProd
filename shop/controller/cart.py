            ...
            if product_check:
                if CartItems.objects.filter(cart=CartHolder, product_id=prod_id):
                    ...
                    cartItem = CartItems.objects.get(
                        product_id=prod_id, cart=CartHolder
                    )
                    cartItem.product_qty = cartItem.product_qty + prod_qty
                    cartItem.save()

                    try:
                        facebook_pixel.AddToCartPixelEvent(
                            request, "NORMAL", product_check, prod_qty
                        )
                    except:
                        pass
                    ...
                    CartItems.objects.create(
                        cart=CartHolder, product_id=prod_id, product_qty=prod_qty
                    )
                    try:
                        facebook_pixel.AddToCartPixelEvent(
                            request, "NORMAL", product_check, prod_qty
        ...
        if cart:
            ...
            if product_check:
                if CartItems.objects.filter(cart=CartHolder, product_id=prod_id):
                    prod_qty = int(request.POST.get("product_qty"))
                    cartItem = CartItems.objects.get(
                        product_id=prod_id, cart=CartHolder, offer_price=offer_price
                    )
                    cartItem.product_qty = cartItem.product_qty + prod_qty
                    cartItem.save()
                    ...
                    try:
                        facebook_pixel.AddToCartPixelEvent(
                            request, "OFFER", product_check, prod_qty, offer_price
                        )
                    except:
                        pass
        ...
        if CartHolder:
            ...
            if product_check and product_check.status == "VARIABLE":
                if CartItems.objects.filter(
                    cart=CartHolder, product_id=prod_id, attribute_id=prod_attr
                ):
                    prod_qty = int(request.POST.get("product_qty"))
                    cartItem = CartItems.objects.get(
                        product_id=prod_id, cart=CartHolder, attribute_id=prod_attr
                    )
                    cartItem.product_qty = cartItem.product_qty + prod_qty
                    cartItem.save()
                    ...
                    try:
                        ...
                        facebook_attribute_name = product_check.title + str(
                        ...
                        facebook_pixel.AddToCartPixelEvent(
                            ...
                            product_check,
                            prod_qty,
                            full_product_attribute_name=facebook_attribute_name,
                            attribute_price=attributeprice,
                            attribute_label=str(newattribute.label),
                        )
                    except:
                        pass
                else:
                    ...
                    CartItems.objects.create(
                        ...
                        attributename=" - " + newattribute.checkattribute,
                        attributeprice=attributeprice,
                    )

                    try:
                        facebook_attribute_name = (
                            product_check.title
                            + " - "
                            + str(newattribute.checkattribute)
                        )
                        facebook_pixel.AddToCartPixelEvent(
                            request,
                            "VARIABLE",
    ...
    if request.method == "POST":
        ...
        if CartItems.objects.filter(cart=CartHolder, product_id=prod_id):
            if prod_attr > 0:
                ...
                cartItem = CartItems.objects.get(
                    product_id=prod_id, cart=CartHolder, attribute=None
                )
                cartItem.delete()
        return JsonResponse({"status": "Updated Successfully!"})
    return redirect("/")
        ...
        if CartHolder:
            ...
            if FeeHolder:
                ...
                if CartFee:
                    ...
                    CartFee = CartFees.objects.create(
                        cart=CartHolder,
                        fee=FeeHolder,
                        title=FeeHolder.title,
                        price=FeeHolder.price,
                    )
