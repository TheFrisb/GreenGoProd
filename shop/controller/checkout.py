    ...
    if request.method == "POST":
        ...
        for fee in neworderfees:
            OrderFeesItem.objects.create(
                order=neworder, fee=fee.fee, title=fee.title, price=fee.price
            )
        ...
        AbandonedCarts.objects.filter(session=request.session["nonuser"]).delete()
        del request.session["nonuser"]
        messages.success(request, "Your order has been placed successfully")
        try:
            facebook_pixel.PurchaseEvent(
                request=request,
                order_items=neworderitems,
                order_total=neworder.total_price,
                number=str(request.POST.get("number")).lower(),
                city=str(request.POST.get("city")).lower(),
        ...
        if orderItem:
            if orderItem.attribute_price is not None:
                ...
                new_item = OrderItem.objects.create(
                    ...
                    price=orderItem.attribute_price
                    - orderItem.attribute_price * 20 // 100,
                ...
                new_item = OrderItem.objects.create(
                    ...
                    - orderItem.product.sale_price * 20 // 100,
                    quantity=product_qty,
            ...
            order.save()
        return JsonResponse(
            {"status": "Product added successfully", "product_id": new_item.id}
        )
