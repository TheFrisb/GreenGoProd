...
def AddToCartPixelEvent(
    request,
    addtocart_type,
    product,
    qty,
    offer_price=None,
    full_product_attribute_name=None,
    ...
    if cartItems:
        for item in cartItems:
            ...
            if item.attributeprice is not None:
                ...
                custom_content_list.append(
                    Content(
                        product_id=str(item.product.id)
                        + "_"
                        + item.attribute.label,  # fix
...
def PurchaseEvent(request, order_items, order_total, number, city, name):
    ...
    user_lastname = ""
    try:
        user_name = splitted_name[0].translate(
    ...
    try:
        user_lastname = splitted_name[1].translate(
            str.maketrans("", "", string.punctuation)
        )
        print(user_lastname)
    except:
