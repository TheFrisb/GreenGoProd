...
class Supplier(models.Model):
    name = models.CharField(
        max_length=150, null=False, blank=False, verbose_name="Име на добавувач"
    )
...
class Product(models.Model):
    ...
    attributes_choices = (("COLOR", "COLOR"), ("SIZE", "SIZE"), ("OFFER", "OFFER"))
    ...
    status = models.CharField(
        choices=status_choices, default="PRIVATE", max_length=50, verbose_name="СТАТУС"
    )
    ...
    free_shipping = models.BooleanField(
        default=False, blank=True, verbose_name="Бесплатна достава"
    )
    date_posted = models.DateTimeField(
        default=timezone.now, verbose_name="Дата на креирање"
    )
    slug = models.SlugField(unique=True, max_length=250, blank=True)
    quantity = models.IntegerField(null=True, blank=True, verbose_name="Залиха")
    attributes_type = models.CharField(
        choices=attributes_choices, max_length=50, blank=True, verbose_name="Одбери тип"
    )
    fake_quantity = models.IntegerField(null=True)
    review_average = models.IntegerField(default=0)
    gallery_is_verified = models.BooleanField(
        default=False, blank=True, verbose_name="Верифицирани слика од производ"
    )
    ...
    supplier = models.ForeignKey(
        Supplier, on_delete=models.CASCADE, verbose_name="Добавувач"
    )
...
class ProductCampaigns(models.Model):
    product = models.ForeignKey(
        Product, on_delete=models.SET_NULL, null=True, verbose_name="Продукт"
    )
...
class ProductGallery(models.Model):
    ...
    galleryimg = ProcessedImageField(
        upload_to="products/product-gallery/%Y/%m/%d/",
        processors=[ResizeToFill(550, 550)],
        format="WEBP",
        options={"quality": 95},
        null=True,
        verbose_name="Слика за галерија",
    )
...
class Offer(models.Model):
    ...
    incentive = models.CharField(
        max_length=100, blank=True, verbose_name="Додатен текст"
    )
...
class ProductAttribute(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.SET_NULL, null=True, verbose_name="Одбери продукт"
    )
    thumbnail = ProcessedImageField(
        upload_to="products_attributes/%Y/%m/%d/",
        processors=[ResizeToFill(550, 550)],
        format="WEBP",
        options={"quality": 95},
        null=True,
    ...
    color = models.ForeignKey(
        Color, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Боја"
    )
    size = models.ForeignKey(
        Size, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Големина"
    )
    offer = models.ForeignKey(
        Offer, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Понуда"
    )
    ...
    def __str__(self):
        return "Атрибут за {} - {} - {}".format(
            self.product, self.checkattribute, self.price
        )
...
class Review(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, blank=True, verbose_name="Продукт"
    )
    ...
    rating = models.CharField(
        choices=rating_choices, default="5", verbose_name="Оценка", max_length=5
    )
    ...
    def __str__(self):
        return "Review за продукт: {} со име: {} и оценка: {}".format(
            self.product.title, self.name, self.rating
        )
...
class CartOffers(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, verbose_name="Одбери продукт"
    )
    offer_text = models.CharField(
        max_length=40, blank=True, verbose_name="Текст на понуда"
    )
    price = models.IntegerField(verbose_name="Цена")
    is_added = models.BooleanField(default=False, verbose_name="Не мени!")

    def __str__(self):
        return "Понуда во кошничка за продукт {} со текст {} IS ADDED {}".format(
            self.product.title, self.offer_text, self.is_added
        )
...
class Order(models.Model):
    ...
    subtotal_price = models.IntegerField(
        null=False, verbose_name="Вкупна цена без достава"
    )
    ...
    message = models.TextField(
        null=True, blank=True, verbose_name="Коментар на муштерија"
    )
    tracking_no = models.CharField(
        max_length=150, null=True, verbose_name="Tracking number"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Креирана во:")
    updated_at = models.DateTimeField(
        auto_now=True, verbose_name="Променет статус:", db_index=True
    )
    ...
    def __str__(self):
        return "{} - Порачка број: {} за {}, {}, со статус {}".format(
            self.id, self.tracking_no, self.name, self.number, self.get_status
        )
...
class OrderItem(models.Model):
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="Креиран во:", null=True
    )
    ...
    def __str__(self):
        return "{}({} ден) х {} - Вкупно {} ден".format(
            self.product, self.price, self.quantity, self.get_product_total
        )
...
class ProductUpsells(models.Model):
    ...
    parent_product = models.ForeignKey(
        Product,
        on_delete=models.CASCADE,
        verbose_name="Продукт на кој да се прикажат",
    ...
    product = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name="Upsell продукт",
    )
    title = models.CharField(max_length=100, verbose_name="Име")
    thumbnail = ProcessedImageField(
        upload_to="upsells/%Y/%m/%d/",
    ...
    regular_price = models.IntegerField(
        verbose_name="Стара цена", blank=True, null=True
    )
