$(document).ready(function () {
  ...
  function isIdInCart(productId, attributeId) {
    const isSimple = attributeId == null || attributeId == 0 || attributeId == "0";
    const idToLookup = isSimple ? `${productId}` : `${productId}-${attributeId}`;

    const val = $("#productIdsInCart").val();
    const productIdsInCart = val ? val.split(",").filter(Boolean) : [];

    return productIdsInCart.includes(idToLookup);
  }

  function addIdInCart(productId, attributeId) {
    const isSimple = attributeId == null || attributeId == 0 || attributeId == "0";
    const idToLookup = isSimple ? `${productId}` : `${productId}-${attributeId}`;
    ...
    if (!productIdsInCart.includes(idToLookup)) {
      productIdsInCart.push(idToLookup);
      $("#productIdsInCart").val(productIdsInCart.join(","));
    }
  }

  function removeIdInCart(productId, attributeId) {
    const isSimple = attributeId == null || attributeId == 0 || attributeId == "0";
    const idToLookup = isSimple ? `${productId}` : `${productId}-${attributeId}`;

    const val = $("#productIdsInCart").val();
    let productIdsInCart = val ? val.split(",").filter(Boolean) : [];

    const index = productIdsInCart.indexOf(idToLookup);
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if ($(button).hasClass("sidecartOfferBtn")) {
          ...
          setTimeout(() => {
            $(button).toggleClass('mainaddedBtn').find(".btn-text").text("ДОДАДЕН")
            $(button).find(".btn-text").css('display', 'inline-block');
            $(button).find(".load-spinner").css('display', 'none');
  ...
  $(document).on('click', '.variable-add-to-cartBtn', function (e) {
    ...
    if ($(selected_attribute).length) {
      ...
      if (isIdInCart(product_id, attribute_id) === true) {
        if (button.hasClass("proceed-to-checkout")) {
          $("#checkout_form_overlay").toggle();
          $('body').toggleClass("checkout-is-active");
        } else {
      ...
      $.ajax({
        ...
        success: function (response) {
          addIdInCart(product_id, attribute_id)
      ...
      $(".attribute-title").css("color", "red").animate({
        'font-size': '16px',
        'font-weight': '700'
      }, 300).animate({'font-size': '12px', 'font-weight': '700'});
    }
  ...
  $('.product-attributes').on('click', '.attribute-item', function () {
    ...
    var $btn = $("button.variable-add-to-cartBtn.baseAddToCartBtn");
    var $textSpan = $btn.find("span.btn-text");
    if (!isIdInCart(product_id, attrib_id)) {

      console.log("no")
      $textSpan.text("ВО КОШНИЧКА")
    } else {
      console.log("yessss")

      $textSpan.text("ДОДАДЕН")
    }
  ...
  $(document).on('click', '.remove-item', function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        removeIdInCart(product_id, attribute_id)
