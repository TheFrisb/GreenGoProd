function create_custom_dropdowns() {
    $('select:not(.form-select)').each(function (i, select) {
        if (!$(this).next().hasClass('dropdown-select')) {
            $(this).after('<div class="dropdown-select wide ' + ($(this).attr('class') || '') + '" tabindex="0"><span class="current" id="currently_selected_campaign"></span><div class="list"><ul></ul></div></div>');
...
$(document).on('click', '.dropdown-select .option', function (event) {
    input = $("#chosen_campaign");
    $(this).closest('.list').find('.selected').removeClass('selected');
    $(this).addClass('selected');
    var text = $(this).data('id');
    var text2 = $(this).data('value')
    $(this).closest('.dropdown-select').find('.current').text(text2);
...
$(document).ready(function () {
  ...
  function get_iso_date(date){
    ...
    $.ajax({
        url: 'https://megazdelka.com/analytics/get-open-audience',
        method: 'GET',
        data: {
    ...
    $(document).on('click', '.dropdown-select .option', function (event) {
        ...
        $.ajax({
            url: 'https://megazdelka.com/analytics/get-campaign-adsets',
            method: 'GET',
            data: {
            ...
            success: function (data) {
                $("#campaign-card-title").text('Адсетови за кампања: ' + campaign_name);
                $("#chosen_product").val(data.product_id)
                var campaign_card = $("#campaign_card_list");
                campaign_card.empty();
                var adsets = data.adsets;
                var adsets_count = adsets.length;
                var adsets_html = "";
                for(var i=0; i<adsets_count; i++){
                    adsets_html += '<li class="list-group-item" id="product_card_label">';
                    adsets_html += '<span style="font-weight:600!important">' + adsets[i].adset_name + '</span> - ' + adsets[i].adset_audience_name;
                    adsets_html += '<button class="btn btn-primary btn-sm copy_existing_adset" data-audience-id="' + adsets[i].adset_audience_id + '" data-audience-name="'+ adsets[i].adset_audience_name +'" type="Button" style="margin-left:10px">Копирај</button>';
                    adsets_html += '</li>';
                    
                }
                campaign_card.append(adsets_html);
            }
        })
    });
    ...
    $(document).on('click', '.submit_media_photo', function(e){
        ...
        if(file == null){
            ...
            $.ajax({
                url: 'https://megazdelka.com/analytics/upload-campaign-photo',
                method: 'POST',
                data: data,
                processData: false,
                contentType: false,
    ...
    $(document).on('click', '.submit_media_video', function(e){
        ...
        if(file == null){
            ...
            data.append('csrfmiddlewaretoken', token);
            $.ajax({
                url: 'https://megazdelka.com/analytics/upload-campaign-video',
                method: 'POST',
                data: data,
                processData: false,
                contentType: false,
                success: function (data) {
                    ...
                    $.ajax({
                        url: 'https://megazdelka.com/analytics/get-video-thumbnails',
                        method: 'GET',
                        data: {
                            'ad_id': ad_id,
                            'csrfmiddlewaretoken': token
    ...
    $(document).on('click', '.search_audiences_btn', function(e){
        ...
        if(search_term == ''){
            ...
            $.ajax({
                url: 'https://megazdelka.com/analytics/search-ad-audiences',
                method: 'GET',
                data: {
    ...
    $(document).on('click', '.video_generate_thumbnail_btn', function(e){
        ...
        $.ajax({
            url: 'https://megazdelka.com/analytics/generate-ad-video-thumbnail',
            method: 'GET',
            data: {
                'ad_id': ad_id,
    ...
    function validate_all_inputs(){
        ...
        if($("#chosen_campaign").val().length == 0){
    ...
    $(document).on('click', '#create_campaign', function(e){
        ...
        if(validate_all_inputs()){
            ...
            $.each(adsets_inputs, function(index, adset){
                ...
                console.log($(adset).find('.adset_start_time_input').val(), ' - ', adset_start_time)
            ...
            $.ajax({
                url: 'https://megazdelka.com/analytics/create-campaign',
                type: 'POST',
                data: {
                    'csrfmiddlewaretoken': token,
                ...
                success: function(response){
                    ...
                    // $.ajax({
                    //     url: 'https://megazdelka.com/analytics/save-new-campaign-id',
                    //     type: 'POST',
                    //     data: {
                    //         'csrfmiddlewaretoken': token,
    ...
    $(document).on("click", ".create_ad_preview" ,function(){
        ...
        console.log('ad_prev')
        $.ajax({
            url: 'https://megazdelka.com/analytics/get-ad-previews',
            type: 'GET',
            data: {
                'csrfmiddlewaretoken': token,
                'ad_primary_text': ad_primary_text,
    ...
    $(document).on("click", "#store_new_audience_btn" ,function(){
        ...
        $.ajax({
            url: 'https://megazdelka.com/analytics/store-new-audience',
            type: 'POST',
            data: {
    ...
    $(document).on("click", ".remove_stored_audience", function(){
        ...
        $.ajax({
            url: 'https://megazdelka.com/analytics/remove-stored-audience',
            type: 'POST',
            data: {
                'csrfmiddlewaretoken': token,
    ...
    $(document).on('click', '.copy_existing_adset', function(){
        var button = $(this);
        var audience_id = $(button).data('audience-id');
        var audience_name = $(button).data('audience-name');
        console.log(audience_id, audience_name)
        console.log('adset_count - ', adset_count)
        if(adset_count == 1){
            var first_adset = $(".adset_template").first();
            var adset_name_input = $(first_adset).find(".adset_name_input");
            var chosen_audience_input = $(first_adset).find(".chosen_ad_audience");
            var aud_id_input = $(first_adset).find(".adset_audience_id_input");
            var aud_name_input = $(first_adset).find(".adset_audience_name_input");
            
            if($(aud_id_input).val().length == 0){
                $(aud_id_input).val(audience_id);
                $(aud_name_input).val(audience_name);
                $(chosen_audience_input).val(audience_name);
                $(adset_name_input).val(audience_name + ' - DUPLICATED');
                return;
            }
            else{
                $(".add_adset").click();
                var last_adset = $(".adset_template").last();
                var adset_name_input = $(last_adset).find(".adset_name_input");
                var chosen_audience_input = $(last_adset).find(".chosen_ad_audience");
                var aud_id_input = $(last_adset).find(".adset_audience_id_input");
                var aud_name_input = $(last_adset).find(".adset_audience_name_input");
                $(aud_id_input).val(audience_id);
                $(aud_name_input).val(audience_name);
                $(chosen_audience_input).val(audience_name);
                $(adset_name_input).val(audience_name + ' - DUPLICATED');
