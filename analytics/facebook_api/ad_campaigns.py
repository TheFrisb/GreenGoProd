...
def create_facebook_ad(
    ...
    if ad_type == "image":
        ...
        creative[AdCreative.Field.url_tags] = (
            "utm_source=facebook+ads&utm_medium={{adset.name}}&utm_campaign={{campaign.name}}&utm_content={{ad.name}}&utm_term={{placement}}"
        )
        ...
        ad = AdAccount(ad_account_id).create_ad(
            ...
            params={
                ...
                "adset_id": ad_set_id,
                "conversion_domain": "megazdelka.com",
        ...
        video_data[AdCreativeVideoData.Field.link_description] = (
            ad_description_text  # Description
        )
...
def search_ad_interests(request, query):
    if request.method == "GET":
        ...
        for interest in interests:
            ...
            estimated_interest_url = (
                f"https://graph.facebook.com/v16.0/{ad_account_id}/reachestimate"
            )
            response = requests.get(estimated_interest_url, params=params)
            if response.status_code == 200:
                response_json = response.json()
                result["users_lower_bound"] = (
                    f"{int(response_json['data']['users_lower_bound']):,}"
                )
                result["users_upper_bound"] = (
                    f"{int(response_json['data']['users_upper_bound']):,}"
                )
                result["adset_name"] = f"{result['name']} " + format_number(
                    response_json["data"]["users_upper_bound"]
                )
...
def create_ad_preview(
    ad_primary_text, ad_description_text, ad_headline_text, photo_url
):
    access_token = config("CAMPAIGNS_SECRET")
    ad_account_id = config("MARKETING_AD_ACCOUNT")
        ...
        "creative": {
            "object_story_spec": {
                "link_data": {
                    "call_to_action": {
                        "type": "SHOP_NOW",
                        "value": {"link": "https://megazdelka.com"},
                    },
                    "description": ad_description_text,
                    "link": "https://megazdelka.com",
...
def get_open_audience(request):
    if request.method == "GET":
        ...
        if response.status_code == 200:
            ...
            result["adset_name"] = f"Open Audience " + format_number(
                response_json["data"]["users_upper_bound"]
            )
        else:
            pass
...
def hehe(parent_campaign):
    ...
    for ad_set in ad_sets:
        ...
        if "flexible_spec" in targeting:
            ...
            FacebookAdSet.objects.create(
                campaign_parent=parent_campaign,
                name=ad_set_name,
                audience_name=audience_name,
                audience_id=audience_id,
            )
            print(
                "Ad Set Name: {}, Audience ID: {}, Audience Name: {}".format(
                    ad_set[AdSet.Field.name], audience_id, audience_name
                )
            )
        else:
            pass
