...
class DailyRow(models.Model):
    owner = models.ForeignKey(
        DailyItems,
        on_delete=models.SET_NULL,
        null=True,
        verbose_name="Дневен ред на информации",
    )
    ...
    neto_price = models.IntegerField(
        verbose_name="Нето вредност",
        null=True,
        blank=True,
    )
    stock_price = models.IntegerField(
        verbose_name="Набавка",
        null=True,
        blank=True,
    )
    fixed_cost = models.IntegerField(
        verbose_name="Фиксен трошок", null=True, blank=True
    )
    ad_cost = models.FloatField(
        verbose_name="Трошок за реклама",
        null=True,
        blank=True,
    )
    neto_total = models.IntegerField(verbose_name="NETO total", null=True, blank=True)
    profit = models.IntegerField(
        verbose_name="Профит(ЌЕШ)",
        null=True,
        blank=True,
    )
    cost_per_purchase = models.FloatField(
        verbose_name="Cost per purchase",
        null=True,
        blank=True,
    )
    be_roas = models.FloatField(
        verbose_name="BE ROAS",
        null=True,
        blank=True,
    )
    ...
    created_at = models.DateTimeField(
        default=timezone.now, editable=True, verbose_name="Креиран во"
    )
    updated_at = models.DateTimeField(
        auto_now=True, verbose_name="Упдејтиран/Импортиран во"
    )
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            ...
            print("Save called")
        super(DailyRow, self).save(*args, **kwargs)
...
class FacebookAd(models.Model):
    main_image = models.ImageField(
        upload_to="campaigns/ads/ad_images/%Y/%m/%d/",
        null=True,
        verbose_name="Слика на реклама",
    )
    main_video = models.FileField(
        upload_to="campaigns/ads/ad_videos/%Y/%m/%d/",
        null=True,
        verbose_name="Видео на реклама",
    )
    video_thumbnail = models.ImageField(
        upload_to="campaigns/ads/ad_videos/thumbnails/%Y/%m/%d/",
        null=True,
        verbose_name="Thumbnail на видео",
    )
...
class StoredAudience(models.Model):
...
class FacebookAdSet(models.Model):
    campaign_parent = models.ForeignKey(
        ProductCampaigns, on_delete=models.SET_NULL, null=True, verbose_name="Кампања"
    )
